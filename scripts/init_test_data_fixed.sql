-- Fixed test data initialization script (compatible with existing schema)
USE charonoms;
...
-- Insert roles (without description field)
INSERT INTO `role` (`id`, `name`, `is_super_admin`, `status`) VALUES
(1, '超级管理员', 1, 0),
(2, '普通管理员', 0, 0),
(3, '操作员', 0, 0)
ON DUPLICATE KEY UPDATE name=VALUES(name), is_super_admin=VALUES(is_super_admin), status=VALUES(status);

-- Update existing admin user's role
UPDATE `useraccount` SET `role_id` = 1 WHERE `username` = 'admin';

-- Insert additional test users (skip if admin already exists)
...
-- Insert menus (without level field - check if it exists first)
-- If 'level' column doesn't exist, we'll omit it
INSERT INTO `menu` (`id`, `name`, `route`, `parent_id`, `sort`, `status`) VALUES
-- Level 1 menus
(1, '系统管理', '', 0, 1, 0),
(2, '学生管理', '', 0, 2, 0),
(3, '教练管理', '', 0, 3, 0),
...
ON DUPLICATE KEY UPDATE name=VALUES(name), route=VALUES(route), parent_id=VALUES(parent_id), sort=VALUES(sort), status=VALUES(status);
...
-- This will give super admin access to everything
