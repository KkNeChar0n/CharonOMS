...
| brandid | int | 外键 → brand.id |
| classifyid | int | 外键 → classify.id |
| isgroup | tinyint | 0=组合商品(套餐), 1=单商品 |
| price | decimal(10,2) | 标准售价 |
| status | tinyint | 0=启用, 1=禁用 |
| create_time | timestamp | 创建时间 |
| update_time | timestamp | 更新时间 |

**goods_attributevalue**（商品-属性值多对多关联）
| 字段 | 类型 | 说明 |
|------|------|------|
| id | int | 主键，自增 |
| goodsid | int | 外键 → goods.id |
| attributevalueid | int | 外键 → attribute_value.id |
| create_time | timestamp | 创建时间 |

**goods_goods**（组合商品-子商品关联）
| 字段 | 类型 | 说明 |
|------|------|------|
| id | int | 主键，自增 |
| goodsid | int | 子商品 ID → goods.id |
| parentsid | int | 父商品(组合) ID → goods.id |
| create_time | timestamp | 创建时间 |
...
1. **attributes 字符串构建**：在仓储层遍历每条商品，查询其关联的属性值，按属性分组后构建格式字符串。attributes_full 为分组后的数组。

2. **isgroup 不可修改**：编辑接口不接受 isgroup 参数，从数据库读取当前值决定是否处理组合逻辑。

3. **组合商品总价计算**：
   ```sql
   SELECT COALESCE(SUM(g.price), 0) as total_price
   FROM goods_goods gg
...
4. **属性绑定全量替换**：
   ```sql
   DELETE FROM goods_attributevalue WHERE goodsid = ?
   INSERT INTO goods_attributevalue (goodsid, attributevalueid) VALUES (?, ?)
   ```

5. **路由顺序约束**：
   - `/goods/active-for-order` 在 `/goods/:id` 之前
   - `/goods/available-for-combo` 在 `/goods/:id` 之前
   - 否则 Gin 会将 `active-for-order` 匹配为 `:id` 参数
